...
| # | 模板 | 类型 | 核心原则 |
|---|------|------|----------|
| 1 | System Prompt 分层 | Prompt | 静态走缓存，动态走拼接 |
| 2 | 四步链路 Prompt | Prompt | 每步职责单一，输出严格 JSON |
| 3 | 澄清/消歧 Prompt | Prompt | 选项点选式，不废话 |
| 4 | Tool(API) 自描述 | YAML | 引擎读它做路由/校验/渲染 |
| 5 | 元数据配置 Schema | YAML | 语义字典 + 时间规则 + 消歧 |
| 6 | 置信度 & 校验阈值 | 配置 | 三档阈值，必须配置化 |
| 7 | 工程化落地 Checklist | 清单 | 5 大类，可直接对照交付 |
| 8 | Go/No-Go 评审清单 | 清单 | 任何一项未通过不得上线 |

---

## 一、System Prompt 分层模板
...
> 把 `{{INJECT}}` 占位部分通过分层 Prompt 工具拼到上面，占位之外的文字保持稳定 → 命中 prompt cache。
...
从用户提问中抽取「指标词 + 维度词 + 时间词 + 排序/对比词」。

输出字段：`intent_type` / `metrics[{raw, confidence}]` / `dimensions` / `time` / `compare` / `rank` / `need_clarify` / `clarify_reason`

约束：
- 不要在 JSON 之外输出任何字符
- 任何字段不确定 → 写 null + 降低 confidence
- "今年/去年/Q1/上半年"等时间词原样填 raw，由规则层处理

### Step 2 · API 路由（仅候选打分）

在候选 API 中，为每个候选给出"是否覆盖用户需求"的判断。
...
基于 API 返回 JSON 撰写 ≤80 字的中文事实摘要。

约束：
- 只描述返回中已有的数字与维度
- 不做趋势预测、不给行动建议
- 不要出现"建议/可能/或许/预计"等推测词
- 数字保留与返回字段相同的小数位
...
当 `need_clarify = true` 时，生成 ≤30 字的中文澄清提问 + 2~4 个可点选选项。
...
每个业务 API 写成以下结构，注册到元数据中心。AI 不直接读它，但**引擎读它做路由/校验/渲染**。
...
关键设计点：
- `aliases` 支持自然语言匹配（"A BU" → `A`）
- `visibility.roles` 控制谁可用，对应 Permission 层
- `validation_rules` 在参数绑定层执行，不等 API 报错才发现
- `related_drilldown` 直接驱动前端下钻 Tab
