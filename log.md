...
## [2026-06-16] create | 飞书Aily智能体平台知识
- 创建4个wiki页面，沉淀飞书Aily企业级AI智能体平台培训内容
- 培训/培训-飞书Aily智能体平台.md — 平台总览：产品矩阵、选型指南、核心功能、MCP集成、7个案例索引
- concepts/飞书Aily-自定义智能体配置.md — 自定义智能体创建全流程：Prompt模板、知识空间、MCP、渠道、数据看板
- queries/飞书Aily-企业智能客服案例.md — 企业智能客服/AI情报/制造业巡检三个案例（完整Prompt+表结构+对话）
- queries/飞书Aily-销售客户回款管理案例.md — 销售管理/客户管理/回款管理/一体化管理四个案例（完整Prompt+表结构+对话）
- 更新index.md：总页面数295→299

## [2026-06-26] create | DAMA-DMBOK2数据管理知识体系Wiki（18页，~430KB）
- 来源：《DAMA数据管理知识体系指南（原书第2版修订版）》中文版DOCX电子版
- 将原DOCX文件（5.1MB）转换为Markdown格式（见 /home/agentuser/dama/markdown_output/）
- 编订18个结构化Wiki页面至 /home/agentuser/wiki/DAMA-DMBOK2/：
  - 00-框架总览.md — DAMA车轮、DMBOK金字塔、10项原则、CDMP认证体系
  - 01-数据管理 — 总纲定义、业务驱动因素、战略一致性模型、阿姆斯特丹信息模型
  - 02-数据处理伦理 — 伦理原则、隐私法律、在线伦理、伦理文化
  - 03-数据治理 — 治理框架、FAIR原则、数据治理委员会
  - 04-数据架构 — 数据架构定义、Zachman/TOGAF映射
  - 05-数据建模和设计 — 概念/逻辑/物理模型、ER图、维度建模
  - 06-数据存储和运营 — 数据库类型、存储架构、数据生命周期
  - 07-数据安全 — 访问控制、加密、安全分类、ISO27001
  - 08-数据集成和互操作 — ETL/ELT、数据虚拟化、语义互操作
  - 09-文档和内容管理 — ECMS、非结构化数据、文档生命周期
  - 10-参考数据和主数据管理 — MDM架构、黄金记录、数据匹配
  - 11-数据仓库和商务智能 — Kimball/Inmon、多维建模、OLAP
  - 12-元数据管理 — 技术/业务/操作元数据、数据目录、数据血缘
  - 13-数据质量管理 — 质量维度、数据剖析、PDCA、DQI
  - 14-大数据和数据科学 — 5V、数据湖、Lambda/Kappa、CRISP-DM
  - 15-数据管理成熟度评估 — CMMI/DCAM/DMM对比
  - 16-数据管理组织和角色期望 — CDO、数据团队模型
  - 17-数据管理和组织变革管理 — ADKAR、Kotter8步法
- 所有页面采用标准模板：YAML frontmatter + 概述→核心概念→方法论（含表格）→组织角色→行业应用→方案价值（风险表格+ROI）→相关页面
- 每页至少10个交叉引用，完整覆盖17个知识领域的关联网络
- 更新index.md添加DAMA-DMBOK2章节入口（总页面数299→317）
