...
updated: 2026-05-14
type: concept
description: '> 相关Wiki：数据库总览 | EBS-DIM接口 | 运营指标体系 报表预处理中间表模块，包含FineReport（fine）报表配置、ST（Statistics）统计中间表和LMST（Lean
  Management Statistics）精益统计中间表。这些表用于预先计算和缓存各类业务指标数据，供FineReport报表和前端看板使用。
