...
IIoT工业物联网模块 — 设备数据采集、监控、传感器管理、边缘计算、数据转发、规则引擎、报警管理、物模型与指令下发等工业物联网核心功能。该模块共计 **76** 张表，涵盖从设备接入（驱动管理、边缘节点）、数据采集与配置（测点模型、地址位）、数据转发（API/MQTT/OPCUA/RabbitMQ）、规则引擎与报警、到消息推送（邮件/企业微信/Webhook）的完整 IIoT 数据链路。

## 表清单

| 表名 | COMMENT注释 |
|------|-------------|
| iiot_agg_0558xlgnhfl5u_boolean | 聚合规则 |
