...
EDO（Electronic Document Order）电子单据模块，负责通威农发设备全生命周期管理及配套单据流程。涵盖设备台账管理（设备台账/设备BOM/设备参数/功能位置）、维保管理（日常/月度/年度维保计划与执行记录、定标管理）、故障管理（故障工单/故障知识库/故障现象/故障原因）、机物料管理（主数据/采购需求计划/采购明细）、文档管理（文档目录/文件）、计量器具与特种设备管理（器具台账/校验执行/安全附件）、人员证件管理（证件台账/检验记录）、固定资产台账、配件建档管理、仓储领料退单、工段状态维护、周会管理以及TPM维保标准等业务功能。
...
- **设备台账管理**：设备台账、设备BOM、设备参数、功能位置、设备附件、设备标准名称、二维码logo
- **维保管理**：日常/月度/年度维保定标、维保计划、维保执行记录、操作记录、附件管理
- **故障管理**：故障工单、故障现象、故障原因、故障类别、故障知识库、机物料消耗记录
- **机物料管理**：机物料主数据、参数属性、采购需求计划、需求明细、采购汇总
- **计量器具管理**：计量器具校验、校验执行附件
- **特种设备管理**：特种设备台账、安全附件、检验执行、人员证件配置
- **人员证件管理**：人员证件台账、检验记录、检验附件
- **文档管理**：文档目录树、文档文件管理
- **资产管理**：固定资产台账、配件建档管理、配件产量记录、工单数据
- **审批流程**：设备审批角色、抄送角色配置、审批流程定义
- **计划与采购**：服务采购需求计划、采购需求资源池、机物料采购计划
- **仓储与领料**：领用单、领用申请附件、申请单明细、退单记录、仓储状态变更日志
- **工段管理**：工段目录、工段状态明细、工段状态维护
- **其他**：设备管理周会、性能测试记录（含锅炉测试）、TPM维保标准、工段停机计划、科目代码配置
